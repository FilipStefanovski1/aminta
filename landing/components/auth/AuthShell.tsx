...
// Aminta is X-first: "Connect" (with the X logo as the icon — no need to
// also spell out the letter, that's redundant) is the primary action (existing
// .rpg-btn-primary treatment), Google stays exactly as it was on
// .rpg-btn-secondary, just below it. No new CSS — both classes already exist.
//
// onX is optional so any future caller that only wants Google keeps working.
// showX/showGoogle default to true so any caller that doesn't pass them
// (there are none today) keeps the pre-flag behavior — visibility is
// controlled by the caller (see AUTH_PROVIDERS in lib/authProviders.ts),
// never hardcoded here, so re-enabling Google is a config change, not a
// component change.
export function OAuthButtons({
  ...
  if (!showX && !showGoogle) return null
  return (
    <div className="space-y-2">
      {showX && onX && (
        <button onClick={onX} type="button"
