...
// phrase-marker anti-slop detector (lib/ai/antiSlop.ts) proved structurally
...
// eval/generation-quality/REPORT.md's v2.1 section). Its real motivating
// failure had no lexical marker to catch at all: Gemini turned a user's own
// "after this event i genuinely think solana is going to dominate consumer
// crypto" (a hedged, future-tense prediction) into "Solana has already won"
// (an unhedged, present-tense claim of fact) — a change in MEANING, not
// vocabulary. This asks the model itself one narrowly scoped question: did
// the meaning change, never "is this good writing."
//
// SOURCE OF TRUTH: extension/lib/claimFidelity.ts (identical duplicate — no
// shared package between extension/ and landing/, same convention as
// lib/ai/antiSlop.ts/lib/ai/prompts.ts). Diff before shipping a change to
// either.
//
// Deliberately plain-JSON-in-prompt, never a schema-forced response, so this
// stays IDENTICAL to the BYOK-side version above (which must run across
// Gemini/Groq/OpenRouter with no schema support on two of the three) — one
// prompt shape, one parser, for both paths.
 ...
 * factual claims. `verifiedFacts` (the admitted context from
 * contextEnrichment.ts's evidence gate, when present) are supplied as
 * additional legitimate source material: a fact drawn from there is not a
 * violation.
 */
...
export function parseFidelityResult(raw: string): FidelityResult {
  try {
    ...
    console.warn("[Included AI] fidelity-check response failed to parse — treating as faithful (fail-open)", {
