// Personal Context reaching the BYOK prompt builder (extension-side
// generation). The Included AI mirror of these assertions lives at
// landing/lib/ai/personalContextPrompt.test.ts — both must stay true, since
// the two apps build prompts from separate copies of prompts.ts.
import { describe, expect, it } from "vitest"
import { buildMessages, buildThreadMessages } from "./prompts"
import type { VoiceProfile } from "./storage"
...
describe("personal context reaches every BYOK generation mode", () => {
  it.each(["tweet", "reply", "polish"] as const)("%s prompts carry it", (mode) => {
    const system = systemOf(buildMessages("x", mode, voice({ personalContext: BACKGROUND }), "some input"))
...
describe("relevance + anti-hallucination instructions travel with it", () => {
  const system = () => systemOf(buildMessages("x", "tweet", voice({ personalContext: BACKGROUND }), "had a good workout today"))
...
describe("absent / legacy profiles", () => {
  it("adds no PERSONAL CONTEXT block at all when the user hasn't written one", () => {
    expect(systemOf(buildMessages("x", "tweet", voice(), "topic"))).not.toContain("PERSONAL CONTEXT (background about this person, written by them")
  })
  ...
  it("a profile saved before the field existed (undefined) builds prompts exactly as before", () => {
    ...
    const system = systemOf(buildMessages("x", "tweet", legacy, "topic"))
  ...
  it("whitespace-only context is treated as empty", () => {
    expect(systemOf(buildMessages("x", "tweet", voice({ personalContext: "   \n  " }), "topic")))
  ...
  it("caps an oversized context so it can't dominate the prompt", () => {
    ...
    const system = systemOf(buildMessages("x", "tweet", voice({ personalContext: huge }), "topic"))
    expect(system).toContain("PERSONAL CONTEXT")
  ...
  it("preserves multi-paragraph background verbatim", () => {
    ...
    expect(systemOf(buildMessages("x", "tweet", voice({ personalContext: paragraphs }), "topic"))).toContain(paragraphs)
  })
})
